sap.ui.define([
  ...
  function showBackendError(error, fallbackMessage) {
    var message = extractBackendMessage(error) ||
      (isAuthorizationError(error) ? "You are not authorized to access Audit Log." : "") ||
      fallbackMessage || "";
    var now = Date.now();
  ...
  function attachModelHandlers(model, key) {
    ...
    try {
      model.attachRequestFailed(function (event) {
        showBackendError(getRequestError(event), "Audit data could not be loaded.");
      });
      model[flag] = true;
    } catch (error) {
  ...
  function attachGlobalHandlers(key) {
    ...
    window.addEventListener("unhandledrejection", function (event) {
      showBackendError(event && event.reason, "Audit request failed.");
    });
  }
